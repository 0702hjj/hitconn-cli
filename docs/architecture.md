...
5. Ask the hidden, versioned agent protocol whether the saved session is
   usable. If not, prefer a one-time `sidTicket` issued by the local CLI
   session or, on macOS, the HITSZ Connect Keychain session. Send only that
   bounded ticket on SSH stdin and redeem it into a distinct target session;
   persistent cookies and SIDs never cross the SSH boundary.
6. If no local session can issue a ticket, run terminal login on a target-side
   SSH TTY. The account, password, and optional SMS code go directly to the
   target process and never enter the orchestrator's process or machine
   protocol.
7. When `--fallback` is explicit, select a WebAgent port free at both ends and
...
8. Validate and temporarily trust only the WebAgent's `127.0.0.1` non-CA leaf,
...
9. The portal sends `sidTicket` through the TLS forward directly to the remote
...
10. Start a transient service, or install and enable it only when `--install`
